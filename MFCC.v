...
void mfccFFT(double *frameSample, double *FFTSample, int frameSize, int pos)
{
	...
	//pos干什么的?
	double dataR[frameSize];//实数
	double dataI[frameSize];//虚数，借用该数组逆序
	for(int i = 0; i < frameSize; i++)
	{
		dataR[i] = frameSample[i + pos];//是将数据以一定的偏置存入吗？
		dataI[i] = 0.0f;//这个赋值什么意思？
	}
	...
	for(i = 0; i < frameSize; i++)
	{
		/*
		i=0,dataI[0]=data[0]
		i=1,dataI[256]=data[1]
		i=2,dataI[128]=data[i]
		i=3,dataI[256+128]=data[i]
		i=4,dataI[64]=data[i]
		i=5,dataI[256+128]=data[i]
		i=6,dataI[128+64]=data[i]
		*/
		...
		dataI[xx] = dataR[i];//以某种规律将数据倒过来存储
	}
	for(i = 0; i < frameSize; i++)
	{
		dataR[i] = dataI[i]; //将补零且逆序过的一帧采样点存入，能保证数据都在2^k位置上，便于FFT？
		dataI[i] = 0; 
	}
...
void computeMel(double mel[400][40], int sampleRate, double *FFTSample, int frameNum, int frameSize)
{
	...
	{
		for(int j = 1; j <= filterNum; j++)//f[j]为第j个滤波器的中心频率
		{
			double temp = 0;
			for(int z = 0; z < frameSize; z++)
			{
				if(z < f[j - 1])
				    ...
				    temp = 0;
				//不理解FFTSample中值的含义
				mel[i][j - 1] += FFTSample[i * frameSize + z] * temp; //依次存储每一帧经过各滤波器的输出结果
			}
		}
    }
